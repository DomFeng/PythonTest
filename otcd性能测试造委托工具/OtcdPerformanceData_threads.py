...
def make_openorder(productNum, productAcctNum, portfolioNum, bstype, exchid, counts):
    ...
    for local_vars.i in range(int(counts)):
        ...
        # 进度条
        # print("\r", end="")
        # progress = round(local_vars.i / int(counts) * 100, 2)
        # print("进度: {}%: ".format(progress), "▓" * (int(local_vars.i / int(counts) * 100) // 2), end=" ")
        # sys.stdout.flush()

        # 判断commit_cns是否等于1000，如果等于1000，提交一次，否则commit_cns+1
        # if local_vars.commit_cns == 1000:
        #     db.commit()
        #     local_vars.commit_cns = 0
        # else:
        #     local_vars.commit_cns += 1
